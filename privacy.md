...
* **Camera Access & Machine Learning:** The App requests access to your device's camera for features such as the "QR Code", "Barcode", "Color Hunt", and "Object Detection" alarms. The camera is used strictly to process images in real-time. We utilize Google ML Kit for on-device machine learning to detect objects and barcodes. All image processing occurs locally on your device. No images, video, or scan data are ever recorded, saved, or transmitted off your device.
* **Physical Activity / Step Counter:** The App requests access to your physical activity data (ACTIVITY_RECOGNITION) exclusively for the "Step Counter" and "Shake" alarm challenges. This data is used in real-time solely to verify movement and dismiss an active alarm. All motion processing happens locally on your device.
...
* **Local Storage & File Backups:** Your alarm configurations (times, challenges, settings) are saved securely in your device's local database. If you use the "Local Backup" feature, a backup file is created locally on your device storage. We do not transmit or access these files.
* **Cloud Backup (Google Sign-In & Firebase):** If you choose to use the "Cloud Backup" feature, we use Google Sign-In to authenticate your account. Your alarm configurations are securely synced to Google Cloud Firestore (Firebase). This data is linked to your Google account identity and is used strictly for the purpose of restoring your alarms across devices. We do not sell or share this data with any third parties.
...
* **Google Play Services & ML Kit:** Used for system security, geofencing, in-app updates, on-device machine learning (ML Kit for object/barcode detection), and the In-App Review API. All ML Kit processing is done on-device and does not send your images to Google servers.
